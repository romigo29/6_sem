...
-- ============================================
-- 1. Vendors (Поставщики ПО) — 8 записей
-- ============================================
...
-- ============================================
-- 2. Locations (Помещения / Классы) — 10 записей
-- ============================================
...
-- ============================================
-- 3. Products (Репозиторий ПО) — 20 записей
-- Категории: Офисное, Разработка, Безопасность,
--            Графика, Учёт, Проектирование, СУБД
-- ============================================
INSERT INTO Products (Vendor_ID, Name, Category, Version) VALUES
-- Microsoft (Vendor_ID = 1)
...
-- Positive Technologies (Vendor_ID = 8)
(8, 'MaxPatrol SIEM',           'Безопасность',   '7.0'),
(8, 'PT Application Firewall',  'Безопасность',   '4.0');
GO

-- ============================================
-- 4. Licenses (Лицензии) — 35 записей
-- Даты покупок распределены с 2023-01 по 2025-03
-- для хорошей аналитики по периодам
-- ============================================
INSERT INTO Licenses (Product_ID, Purchase_Date, Expiration_Date, Unit_Price, Total_Seats) VALUES
-- Windows 11 Pro (Product_ID = 1)
(1,  '2023-01-15', '2026-01-15', 12500.00, 30),
(1,  '2023-07-10', '2026-07-10', 12500.00, 20),
(1,  '2024-03-01', '2027-03-01', 13500.00, 15),

-- Office 365 (Product_ID = 2)
(2,  '2023-02-01', '2024-02-01', 5500.00,  30),
(2,  '2024-02-05', '2025-02-05', 5800.00,  30),
(2,  '2025-02-10', '2026-02-10', 6200.00,  35),

-- Visual Studio (Product_ID = 3)
(3,  '2023-03-20', '2026-03-20', 45000.00, 10),
(3,  '2024-06-15', '2027-06-15', 48000.00, 10),

-- SQL Server Developer (Product_ID = 4)
(4,  '2023-04-10', NULL,          0.00,     50),

-- Adobe Photoshop (Product_ID = 5)
(5,  '2023-05-01', '2024-05-01', 18000.00, 10),
(5,  '2024-05-05', '2025-05-05', 19500.00, 12),
(5,  '2025-01-15', '2026-01-15', 20000.00, 5),

-- Adobe Illustrator (Product_ID = 6)
(6,  '2023-06-01', '2024-06-01', 18000.00, 8),
(6,  '2024-08-20', '2025-08-20', 19000.00, 10),

-- Adobe Premiere Pro (Product_ID = 7)
(7,  '2024-01-10', '2025-01-10', 22000.00, 5),
(7,  '2025-01-15', '2026-01-15', 23500.00, 5),

-- IntelliJ IDEA (Product_ID = 8)
(8,  '2023-09-01', '2024-09-01', 25000.00, 15),
(8,  '2024-09-05', '2025-09-05', 27000.00, 15),

-- PyCharm (Product_ID = 9)
...
-- 1С:Предприятие (Product_ID = 13)
(13, '2023-08-01', NULL,         35000.00,  5),

-- 1С:Бухгалтерия (Product_ID = 14)
...
-- ============================================
-- 6. Installations (Установки ПО) — 60 записей
-- Разная активность и Usage_Count для аналитики
-- ============================================
INSERT INTO Installations (Device_ID, License_ID, Last_Activity_Date, Usage_Count) VALUES
-- Windows 11 на ПК в аудиториях 101, 102
(1,  1, '2025-03-28', 450),
(2,  1, '2025-03-27', 430),
(3,  1, '2025-03-28', 410),
(4,  2, '2025-03-26', 380),
...
-- Adobe Photoshop на ПК в 203
(17, 10, '2025-03-22', 85),
(18, 11, '2025-03-28', 92),
(19, 11, '2025-03-27', 78),
(20, 12, '2025-02-10', 45),

-- Adobe Illustrator на ПК
(17, 13, '2025-03-20', 60),
(18, 14, '2025-03-25', 72),
(19, 14, '2025-03-28', 55),

-- Adobe Premiere Pro на Ноутбуках 202
(14, 15, '2025-03-18', 30),
(15, 16, '2025-03-28', 42),

-- IntelliJ IDEA на ПК в 201
(9,  17, '2025-03-10', 200),
...
-- DataGrip на Ноутбуках 302
(24, 21, '2025-03-20', 60),
(25, 21, '2025-03-28', 55),

-- Kaspersky Endpoint — на всех ПК
(1,  22, '2025-03-28', 500),
(2,  22, '2025-03-28', 490),
(3,  22, '2025-03-28', 480),
(6,  23, '2025-03-28', 470),
...
-- SQL Server на серверах
(26, 9, '2025-03-28', 1000),
(27, 9, '2025-03-28', 950),

-- Oracle Database на сервере
(28, 33, '2025-03-28', 800),

-- MaxPatrol SIEM на сервере
(26, 34, '2025-03-28', 600),

-- PT Application Firewall на сервере
(27, 35, '2025-03-28', 550);
GO

-- ============================================
-- 7. Requests (Заявки на ПО) — 25 записей
-- Разные статусы и даты
-- ============================================
...
BEGIN
    ...
    SELECT 
        ...
        CASE 
            WHEN MONTH(l.Purchase_Date) <= 6 THEN 1
            ELSE 2
        END AS HalfYear,
    ...
    GROUP BY GROUPING SETS (
        (p.Name, YEAR(l.Purchase_Date), MONTH(l.Purchase_Date)),
        (p.Name, YEAR(l.Purchase_Date), DATEPART(QUARTER, l.Purchase_Date)),
        (p.Name, YEAR(l.Purchase_Date), 
            CASE WHEN MONTH(l.Purchase_Date) <= 6 THEN 1 ELSE 2 END),
        (p.Name, YEAR(l.Purchase_Date))
    )
...
BEGIN
    SELECT
        ...
        -- Доля от общего количества лицензий (%)
        CAST( COUNT(*) * 100.0 / SUM(COUNT(*)) OVER ()
        AS DECIMAL(5,2)) AS [% от общего кол-ва],
 
        -- Доля от общей стоимости лицензий (%)
...
EXEC sp_LicenseCostAnalysis;                                -- все данные
EXEC sp_LicenseCostAnalysis '2024-01-01', '2024-12-31';    -- только 2024 год
GO


-- ============================================
-- Задание 5: ROW_NUMBER() для пагинации
-- Выводим заданную страницу по 20 строк
-- ============================================

USE SoftwareLicenseManagement;
GO
...
-- ============================================
-- Задание 6: ROW_NUMBER() для удаления дубликатов
-- Оставляем только последнюю заявку пользователя
-- на каждый продукт
-- ============================================
...
-- ============================================
-- Задание 8: Какой тип ПО (категория) использовался
